    ...
    class Order_Form_API_KEYS extends WP_REST_Controller
    {
        ...
        public function create_update_key($request)
        {
            ...
            if (isset($request['data']) && isset($request['data']['consumer_key'])) {
                ...
                update_option('wwof_order_form_v2_consumer_key', sanitize_text_field($request['data']['consumer_key']));
                update_option('wwof_order_form_v2_consumer_secret', sanitize_text_field($request['data']['consumer_secret']));

                $data = array(
                    'status'  => 'success',
                    'message' => __('API keys updated.', 'woocommerce-wholesale-order-form'),
                );
                ...
                $data = array(
                    'status'  => 'fail',
                    'message' => __('Unable to update API Keys.', 'woocommerce-wholesale-order-form'),
                );
