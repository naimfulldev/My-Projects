jQuery(document).ready(function ($) {
  ...
  var $surcharge_controls = $(
      "#payment-gateway-surcharge-mapping .surcharge-controls"
    ),
    ...
    $wwpp_surcharge_taxable = $surcharge_controls.find(
      "#wwpp-surcharge-taxable"
    ),
    $button_controls = $("#payment-gateway-surcharge-mapping .button-controls"),
    $wholesale_payment_gateway_surcharge = $(
      "#wholesale-payment-gateway-surcharge"
    ),
  ...
  $button_controls.find("#add-surcharge").click(function () {
    ...
    if (error_fields.length > 0) {
      var msg =
        wwpp_payment_gateway_controls_custom_field_params.i18n_specify_field_values_with_colon +
        "<br/><ul>";
      ...
      toastr.error(
        msg,
        wwpp_payment_gateway_controls_custom_field_params.i18n_form_error,
    ...
    var surchargeData = {
      ...
      surcharge_type_text: $.trim(
        $wwpp_surcharge_type.find(":selected").text()
      ),
    ...
    wwppBackendAjaxServices
      .addPaymentGatewaySurcharge(
        surchargeData,
        wwpp_payment_gateway_controls_custom_field_params.user_id
      )
      .done(function (data, textStatus, jqXHR) {
        console.log(data, surchargeData);
        if (data.status == "success") {
          toastr.success(
            "",
            wwpp_payment_gateway_controls_custom_field_params.i18n_payment_gateway_added,
          ...
          $wholesale_payment_gateway_surcharge
            ...
            .append(
              '<tr class="' +
                ...
                $wwpp_wholesale_roles
                  .find("option[value='" + surchargeData.wholesale_role + "']")
                  .text() +
                "</td>" +
                '<td class="payment-gateway-text">' +
                $wwpp_payment_gateway
                  .find("option[value='" + surchargeData.payment_gateway + "']")
                  .text() +
                "</td>" +
                ...
                $wwpp_surcharge_type
                  .find("option[value='" + surchargeData.surcharge_type + "']")
                  .text() +
                "</td>" +
    ...
    wwppBackendAjaxServices
      ...
      .done(function (data, textStatus, jqXHR) {
        if (data.status == "success") {
          ...
          toastr.success(
            "",
            wwpp_payment_gateway_controls_custom_field_params.i18n_payment_gateway_updated,
            { closeButton: true, showDuration: successMessageDuration }
          );
        } else {
          toastr.error(
  ...
  $wholesale_payment_gateway_surcharge.delegate(".edit", "click", function () {
    ...
    var currMapping = {
      ...
      wholesale_role: $.trim(
        $currentTr.find(".meta").find(".wholesale-role").text()
      ),
      payment_gateway: $.trim(
        $currentTr.find(".meta").find(".payment-gateway").text()
      ),
      surcharge_title: $.trim($currentTr.find(".surcharge-title").text()),
      surcharge_type: $.trim(
        $currentTr.find(".meta").find(".surcharge-type").text()
      ),
    ...
    $wwpp_wholesale_roles
      .val(currMapping.wholesale_role)
      .trigger("change")
      .trigger("chosen:updated");
    $wwpp_payment_gateway
      .val(currMapping.payment_gateway)
      .trigger("change")
      .trigger("chosen:updated");
    $wwpp_surcharge_title.val(currMapping.surcharge_title);
    $wwpp_surcharge_type
      .val(currMapping.surcharge_type)
      .trigger("change")
      .trigger("chosen:updated");
    $wwpp_surcharge_amount.val(currMapping.surcharge_amount);
    $wwpp_surcharge_taxable
      .val(currMapping.taxable)
      .trigger("change")
      .trigger("chosen:updated");
  ...
  $wholesale_payment_gateway_surcharge.delegate(
    ".delete",
    "click",
    function () {
      var $this = $(this),
      ...
      if (
        confirm(
          wwpp_payment_gateway_controls_custom_field_params.i18n_click_ok_remove_payment_gateway
        )
      ) {
          ...
          .done(function (data, textStatus, jqXHR) {
            if (data.status == "success") {
              $currentTr.fadeOut("fast", function () {
                ...
                if (
                  $wholesale_payment_gateway_surcharge.find("tbody").find("tr")
                    .length <= 0
                ) {
                  $wholesale_payment_gateway_surcharge
                    .find("tbody")
              ...
              toastr.success(
                "",
                wwpp_payment_gateway_controls_custom_field_params.i18n_payment_gateway_deleted,
                { closeButton: true, showDuration: successMessageDuration }
              );
            } else {
              toastr.error(
  ...
  $surcharge_controls
    .find("#wwpp-payment-gateway")
    .chosen({ allow_single_deselect: true });
  $surcharge_controls
    .find("#wwpp-surcharge-type")
    .chosen({ allow_single_deselect: true });
  $surcharge_controls
    .find("#wwpp-surcharge-taxable")
    .chosen({ allow_single_deselect: true });
});
