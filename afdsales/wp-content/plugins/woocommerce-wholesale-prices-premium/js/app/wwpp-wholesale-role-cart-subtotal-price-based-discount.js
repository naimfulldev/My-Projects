jQuery(document).ready(function ($) {
  ...
  var $wrapper = $("#wholesale-role-cart-total-based-discount-container"),
    ...
    $mapping_table = $wrapper.find(
      "#wholesale-role-cart-total-discount-mapping"
    ),
  ...
  function construct_mapping_data(
    ...
    if (wholesale_role == "")
      err.push(
        cart_subtotal_price_based_discount_params.i18n_please_specify_wholesale_role
      );

    if (subtotal_price <= 0 || subtotal_price == "" || isNaN(subtotal_price))
      err.push(
        cart_subtotal_price_based_discount_params.i18n_invalid_subtotal_price
      );

    if (discount_type == "")
      err.push(
        cart_subtotal_price_based_discount_params.i18n_invalid_discount_type
      );

    if (discount_amount <= 0 || discount_amount == "" || isNaN(discount_amount))
      err.push(
        cart_subtotal_price_based_discount_params.i18n_invalid_discount_amount
      );

    if (discount_title == "")
      err.push(
        cart_subtotal_price_based_discount_params.i18n_invalid_discount_title
      );
  ...
  function prepopulate_form_with_entry_data(
    $tr,
    $field_controls,
    $button_controls
  ) {
    var type =
      $.trim($tr.find(".discount_type").text()) == "Percent Discount"
        ? "percent-discount"
        : "fixed-discount";
    ...
    $field_controls
      .find("#subtotal-price")
      .val($.trim($tr.find(".subtotal_price").text()));
    $field_controls.find("#discount-type").val(type).trigger("chosen:updated");
    $field_controls
      .find("#discount-amount")
      .val($.trim($tr.find(".discount_amount").text().replace("%", "")));
    $field_controls
      .find("#discount-title")
      .val($.trim($tr.find(".discount_title").text()));
  ...
  function add_new_mapping_table_entry(index, data, $mapping_table) {
    let discount_amount =
      data.discount_type == "percent-discount"
    ...
    var entry_str =
      ...
      cart_subtotal_price_based_discount_params.wholesale_roles[
        data.wholesale_role
      ].roleName +
      "</td>" +
      ...
      cart_subtotal_price_based_discount_params.i18n_discount_type[
        data.discount_type
      ] +
      "</td>" +
  ...
  $add_mapping.on("click", function (e) {
    ...
    if (result.status !== "success") {
      ...
      toastr.error(
        err_msg,
        cart_subtotal_price_based_discount_params.i18n_form_error,
        {
          closeButton: true,
      ...
      $.ajax({
        ...
        .done(function (data) {
          if (data.status == "success") {
            toastr.success(
              "",
              cart_subtotal_price_based_discount_params.i18n_role_successfully_added,
              { closeButton: true, showDuration: successMessageDuration }
            );
            add_new_mapping_table_entry(
  ...
  $mapping_table.on("click", "tbody td.controls .delete", function (e) {
    ...
    if (
      confirm(
        cart_subtotal_price_based_discount_params.i18n_confirm_remove_mapping
      )
    ) {
      $cancel_save_mapping
      ...
      $.ajax({
        ...
        .done(function (data) {
          if (data.status === "success") {
            toastr.success(
              "",
              cart_subtotal_price_based_discount_params.i18n_role_successfully_deleted,
              { closeButton: true, showDuration: successMessageDuration }
            );
      ...
      $.ajax({
        ...
        .done(function (data) {
          if (data.status == "success") {
            toastr.success(
              "",
              cart_subtotal_price_based_discount_params.i18n_role_successfully_updated,
              { closeButton: true, showDuration: successMessageDuration }
            );

            edit_mapping_table_entry(
              result.data.index,
              result.data,
              $mapping_table
            );
