jQuery(document).ready(function ($) {
  ...
  var $wrcqbwdc = $(
      "#wholesale-role-cart-qty-based-wholesale-discount-container"
    ),
    ...
    $wrcqbwdc_cancel_save_mapping = $wrcqbwdc.find(
      "#wrcqbwd-cancel-edit-mapping"
    ),
    $wrcqbwdc_mapping_table = $wrcqbwdc.find(
      "#wholesale-role-cart-qty-based-wholesale-discount-mapping"
    ),
  ...
  function wrcqbwdc_construct_mapping_data(
    wholesale_role,
    start_qty,
    end_qty,
    discount,
    ...
    if (start_qty <= 0 || start_qty == "" || isNaN(start_qty))
      err.push(wwpp_wrcqbwd_params.i18n_invalid_start_qty);

    if (end_qty === 0 || isNaN(end_qty) || end_qty < 0)
      err.push(wwpp_wrcqbwd_params.i18n_invalid_end_qty);

    if (discount <= 0 || discount == "" || isNaN(discount))
  ...
  function wrcqbwdc_prepopulate_form_with_entry_data(
    $tr,
    $field_controls,
    $button_controls
  ) {
    ...
    $field_controls
      .find("#wrcqbwd-starting-qty")
      .val($.trim($tr.find(".start_qty").text()));
    $field_controls
      .find("#wrcqbwd-ending-qty")
      .val($.trim($tr.find(".end_qty").text()));
    $field_controls
  ...
  $wrcqbwdc_add_mapping.click(function (e) {
    ...
    if (result.status !== "success") {
      ...
      wrcqbwdc_form_in_processing_mode(
        $wrcqbwdc_field_controls,
        $wrcqbwdc_button_controls
      );
      ...
      $.ajax({
        ...
        .done(function (data) {
          if (data.status == "success") {
            wrcqbwdc_add_new_mapping_table_entry(
  ...
  $wrcqbwdc_mapping_table.on(
    "click",
    "tbody td.controls .delete",
    function (e) {
      e.preventDefault();
      ...
      if (confirm(wwpp_wrcqbwd_params.i18n_confirm_remove_mapping)) {
        ...
        $.ajax({
          ...
          .done(function (data) {
            if (data.status === "success")
              wrcqbwdc_remove_mapping_table_entry(
                index,
                $wrcqbwdc_mapping_table
              );
            else {
  ...
  $wrcqbwdc_mapping_table.on("click", "tbody td.controls .edit", function (e) {
    ...
    wrcqbwdc_prepopulate_form_with_entry_data(
      $tr,
      $wrcqbwdc_field_controls,
      $wrcqbwdc_button_controls
    );
  ...
  $wrcqbwdc_save_mapping.click(function (e) {
    ...
    if (result.status !== "success") {
      ...
      $.ajax({
        ...
        .done(function (data) {
          if (data.status == "success") {
            wrcqbwdc_edit_mapping_table_entry(
              result.data.index,
              result.data,
              $wrcqbwdc_mapping_table
            );
            ...
            toastr.error(
              data.error_msg,
              wwpp_wrcqbwd_params.i18n_edit_mapping_error,
              { closeButton: true, showDuration: error_message_duration }
            );
            console.log(data);
          }
        ...
        .fail(function (jqxhr) {
          toastr.error(
            wwpp_wrcqbwd_params.i18n_failed_edit_mapping,
            wwpp_wrcqbwd_params.i18n_edit_mapping_error,
            { closeButton: true, showDuration: error_message_duration }
          );
          console.log(jqxhr);
