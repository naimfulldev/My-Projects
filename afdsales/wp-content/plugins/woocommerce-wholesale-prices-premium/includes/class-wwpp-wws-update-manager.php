...
class WWPP_WWS_Update_Manager
{
     ...
     * @since 1.17
     * @since 1.26.5
     * We added new parameter $force. This will serve as a flag if we are going to "forcefully" fetch the latest update data from the server.
     *
    ...
    public function inject_plugin_update_info($result, $action, $args)
    {
        ...
        if ($license_activated === 'yes' && $action == 'plugin_information' && isset($args->slug) && $args->slug == 'woocommerce-wholesale-prices-premium') {
     ...
     * we inject our plugin update data in, if any. It is saved on WWPP_UPDATE_DATA option.
    ...
    public function after_plugin_update($upgrader_object, $options)
    {
        ...
        if ($options['action'] == 'update' && $options['type'] == 'plugin' && isset($options['plugins']) && is_array($options['plugins'])) {
            ...
            foreach ($options['plugins'] as $each_plugin) {
                ...
                if ($each_plugin == WWPP_PLUGIN_BASE_NAME) {
    ...
    {
        if (isset($item->plugin) && WWPP_PLUGIN_BASE_NAME === $item->plugin) {
            return false;
        }
    ...
    public function wwpp_force_fetch_update_data()
    {
        ...
        if ($update_data) {
            ...
            if ($update_transient && isset($update_transient->response) && is_array($update_transient->response)) {
