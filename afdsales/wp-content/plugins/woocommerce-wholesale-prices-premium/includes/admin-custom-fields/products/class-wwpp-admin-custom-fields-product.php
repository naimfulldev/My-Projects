    ...
    class WWPP_Admin_Custom_Fields_Product {
        ...
        */
        ...
        private function _print_order_quantity_based_wholesale_pricing_controls( $product_id , $wholesale_roles , $classes ) {
            ...
            if( $classes == 'variable' ) {
                ...
                if( $variable_qty_based_disc_enabled == 'yes' )
            ...
            if( $hide_variation_qty_based_discount || in_array( WWP_Helper_Functions::wwp_get_product_type( $product ) , array( 'simple' , 'variation' ) ) )
                $is_variable_parent_flag = true; ?>

            <div class="product-quantity-based-wholesale-pricing options_group <?php echo $classes; ?>" style="display:<?php echo $hide_variation_qty_based_discount ? 'none;': 'block;';?>">
                    ...
                    <p class="pqbwp-desc">
                        <?php
                            if( !$is_variable_parent_flag ) { 
                                _e( 'Specify further wholesale discounts based on the total quantity of all variations of this product in the cart. If you wish to use quantity based pricing on individual variations, uncheck this option and use the controls on the variations instead. <b>Ending Qty</b> can be left blank to apply that price for all quantities above the <b>Starting Qty.</b> Only applies to the wholesale roles that you specify. <br/><b>Note: Wholesale Price is calculated via percentage discount, it is based on the wholesale price of variation if set else it will use the regular price.</b>' , 'woocommerce-wholesale-prices-premium' ); 
                            } else {
                                _e( 'Specify wholesale price for this current product depending on the quantity being purchased.<br><b>Ending Qty</b> can be left blank to apply that price for all quantities above the <b>Starting Qty.</b><br/>Only applies to the wholesale roles that you specify.' , 'woocommerce-wholesale-prices-premium' ); 
                            } ?>
                    </p>
                ...
                <p class="form-field pqbwp-enable-field-container">
                    ...
                    <input type="hidden" name="wwpp_post_meta_enable_quantity_discount_rule_<?php echo $product_id; ?>" class="wwpp_post_meta_enable_quantity_discount_rule" value="<?php echo $pqbwp_enable ?>">
                    <input type="checkbox" name="pqbwp-enable" class="pqbwp-enable checkbox" value="yes" <?php echo ( $pqbwp_enable == 'yes' ) ? 'checked' : ''; ?> autocomplete="off">
        ...
        private function _print_mapping_item( $item_number , $index , $map , $wholesale_roles , $wholesale_role_meta_key , $wholesale_role_price_key , $start_qty_meta_key , $end_qty_meta_key , $price_type_meta_key , $aelia_currency_switcher_active , $args , $currency = null ) {
            ...
            <tr class="<?php echo $row_class; ?>">
                ...
                <td class="meta hidden">
                    ...
                    <span class="price-type"><?php echo isset( $map[ $price_type_meta_key ] ) ? $map[ $price_type_meta_key ] : 'fixed-price'; ?></span>
                ...
                <td class="wholesale-role-text"><?php
                    if ( isset( $wholesale_roles[ $map[ $wholesale_role_meta_key ] ][ 'roleName' ] ) )
                        echo $wholesale_roles[ $map[ $wholesale_role_meta_key ] ][ 'roleName' ];
                    else
                        echo sprintf( __( '%1$s role does not exist anymore' , 'woocommerce-wholesale-prices-premium' ) , $map[ $wholesale_role_meta_key ] ); ?>
                </td>
                ...
                <td class="wholesale-price-text">
                    ...
                    if ( isset( $map[ $price_type_meta_key ] ) ) {
                        ...
                        if ( $map[ $price_type_meta_key ] == 'fixed-price' )  
                            echo WWP_Helper_Functions::wwp_formatted_price( $map[ $wholesale_role_price_key ] , $args );
                        elseif ( $map[ $price_type_meta_key ] == 'percent-price' )
                            echo str_replace( get_woocommerce_currency_symbol() , '' , WWP_Helper_Functions::wwp_formatted_price( $map[ $wholesale_role_price_key ] , $args ) ) . '%';
        ...
        */
        ...
        public function wc_export_meta_value_filter( $value , $meta , $product , $row ) {
            ...
            if( $meta->key == 'wwpp_post_meta_quantity_discount_rule_mapping' ) {
        ...
        public function save_wwpp_post_meta_enable_quantity_discount_rule( $post_id , $post , $update ) {
            ...
            if( isset( $_POST[ WWPP_POST_META_ENABLE_QUANTITY_DISCOUNT_RULE . '_' . $post_id ] ) )
                update_post_meta( $post_id , WWPP_POST_META_ENABLE_QUANTITY_DISCOUNT_RULE , $_POST[ WWPP_POST_META_ENABLE_QUANTITY_DISCOUNT_RULE . '_' . $post_id ] );
        ...
        public function run() {
            ...
            // WWPP-592 Set qty based wholesale discount on the parent variable level
