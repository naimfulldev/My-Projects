...
if (!class_exists('WWPP_Wholesale_Role_General_Discount_Mapping')) {
    ...
    class WWPP_Wholesale_Role_General_Discount_Mapping {
        ...
        public function ajax_add_wholesale_role_cart_subtotal_discount_mapping() {
            ...
            if (!defined('DOING_AJAX') || !DOING_AJAX) {
                $response = array('status' => 'fail', 'error_msg' => __('Invalid ajax request', 'woocommerce-wholesale-prices-premium'));
            } elseif (!isset($_POST['cart_total_based_discount_mapping'])) {
                $response = array('status' => 'fail', 'error_msg' => __('Required data not supplied', 'woocommerce-wholesale-prices-premium'));
            } else {

                // Sanitize
                $rule = $_POST['cart_total_based_discount_mapping'];
                $rule = array_map('esc_attr', $rule);

                if (!empty($rule['min_total_price']) && $rule['min_total_price'] <= 0) {
                    return array('status' => 'fail', 'error_msg' => __('Total price must not be less than or equal to 0.', 'woocommerce-wholesale-prices-premium'));
                }

                if (!empty($rule['discount_amount']) && $rule['discount_amount'] <= 0) {
                    return array('status' => 'fail', 'error_msg' => __('Discount amount must not be less than or equal to 0.', 'woocommerce-wholesale-prices-premium'));
                }
